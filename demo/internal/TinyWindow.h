...
namespace TinyWindow
{
    ...
    public:
        ...
        void ShutDown()
        {
            //windowList.empty();
    #if defined(TW_WINDOWS)
            // [UAA] Avoid resetting every display on shutdown. Per-window
            // fullscreen teardown happens in `ShutdownWindow`.
    #elif defined(TW_LINUX)
            Linux_Shutdown();
    #endif
    ...
    private:
        ...
        void ShutdownWindow(tWindow* window)
        {
            ...
            // [UAA] Restore fullscreen mode only for the monitor this window
            // actually used, instead of resetting every display on exit.
            if (window->settings.currentState == state_t::fullscreen && window->currentMonitor != nullptr)
            {
                ChangeDisplaySettingsEx(window->currentMonitor->displayName.c_str(), nullptr, nullptr, CDS_FULLSCREEN, nullptr);
            }
            ...
            if (window->windowHandle && window->deviceContextHandle)
            {
                ReleaseDC(window->windowHandle, window->deviceContextHandle);
            }
            if (window->windowHandle)
            {
                // [UAA] Tear down the HWND explicitly before unregistering the class.
